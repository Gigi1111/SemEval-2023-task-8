...
file_name = os.path.join(__location__, "per_exp.csv")
...
#torch.save(X_g, "X_per_exp.pt")

X = torch.load(os.path.join(__location__, "X_per_exp.pt"))
print(len(X))
...
#Generate test sets
X_test = X[350000:]
Y_test = Y[350000:].view(len(Y[350000:]))
...
torch.save(model.state_dict(), os.path.join(__location__, 'model_bin_pe_10ep'))
...
model = md.Linear(768,1)
model.load_state_dict(torch.load(os.path.join(__location__, 'model_bin_pe_10ep')))
model.eval()
preds = []
for x in X_test:
