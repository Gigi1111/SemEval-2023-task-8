...
file_name = os.path.join(__location__, "clean_claims.csv")
...
#torch.save(X_g, "X_claim.pt")

X = torch.load(os.path.join(__location__,"X_claim.pt"))
print(len(X))
...
#Generate test sets
X_test = X[24500:]
Y_test = Y[24500:].view(len(Y[24500:]))
...
torch.save(model.state_dict(), os.path.join(__location__, 'model_bin_c_30ep'))
...
model = md.Linear(768,1)
model.load_state_dict(torch.load(os.path.join(__location__, 'model_bin_c_30ep')))
model.eval()
preds = []
for x in X_test:
